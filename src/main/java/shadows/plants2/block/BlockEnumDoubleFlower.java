...
public class BlockEnumDoubleFlower<E extends Enum<E> & IFlowerEnum> extends BlockEnumFlower<E> {
    ...
    public IBlockState getStateFromMeta(int meta) {
        int k = meta % 2;
        if (k == 0)
            return getDefaultState().withProperty(UPPER, true).withProperty(property, types.get(getActualMeta(meta)));
        if (k == 1)
            return getDefaultState().withProperty(UPPER, false).withProperty(property, types.get(getActualMeta(meta)));
        else return null;
    }
    ...
    public void initRecipes(Register<IRecipe> event) {
        for (E e : getTypes()) {
            if (e.useForRecipes())
                Plants2.HELPER.addShapeless(PlantUtil.getDyeForEnum(e.getColor(), 3), new ItemStack(this, 1, e.getMetadata()));
        }
    }
