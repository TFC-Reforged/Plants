...
public class BlockFlowerpot extends BlockFlowerPot implements IHasModel, IItemBlock {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState flowerpotState, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (flowerpot.isEmpty()) {
            ...
            if (toUse.getBlock() != this && (toUse.getBlock() == Blocks.AIR || toUse.getBlock().hasTileEntity(toUse)))
                return false;
            if (BlockEnumHarvestBush.FRUIT.equals(toUse.getBlock().getBlockState().getProperty("fruit")))
                toUse = toUse.withProperty(BlockEnumHarvestBush.FRUIT, true);
            pot.setState(toUse);
    ...
    public IBlockState getExtendedState(IBlockState state, IBlockAccess world, BlockPos pos) {
        ...
        if (t != null && state instanceof IExtendedBlockState)
            return ((IExtendedBlockState) state).withProperty(UnlistedStateProperty.UNLISTED_STATE, t.getState());
        return state;
    }
