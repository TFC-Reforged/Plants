...
public class BlockEnumHarvestBush<E extends Enum<E> & IHarvestableEnum> extends BlockEnumBush<E> implements IGrowable {
    ...
    public IBlockState getStateFromMeta(int meta) {
        int k = meta % 2;
        if (k == 0)
            return getDefaultState().withProperty(FRUIT, true).withProperty(property, types.get(getActualMeta(meta)));
        if (k == 1)
            return getDefaultState().withProperty(FRUIT, false).withProperty(property, types.get(getActualMeta(meta)));
        else return null;
    }
