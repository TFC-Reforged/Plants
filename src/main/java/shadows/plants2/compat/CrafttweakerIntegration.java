    ...
    public static void processCauldronChanges() {
        ...
        for (Pair<EnumDyeColor, Potion> p : removals) {
            ...
            if (pot != null) list.remove(pot);
                ...
                CraftTweakerAPI.logError("[Plants] Tried to remove a weighted potion entry, but no entry was found. Color: " + p.getLeft() + ", Potion: " + p.getRight().getRegistryName());
        ...
        for (Map.Entry<EnumDyeColor, List<WeightedPotion>> ent : ColorToPotionUtil.MAP.entrySet())
            if (ent.getValue().isEmpty())
                throw new RuntimeException("[Plants] All WeightedPotions have been removed from the " + ent.getKey() + " list, this is not allowed!");
