...
public class Decorator {
    ...
    public void recieveGenEvent(DecorateBiomeEvent.Decorate event) {
        ...
        if (PlantConfig.COMPUTED_BIOME_BL.contains(event.getWorld().getBiome(event.getChunkPos().getBlock(0, 0, 0))))
            return;
        genFlowers(event);
        ...
        if (PlantConfig.vineGen && !event.getWorld().isRemote && event.getRand().nextInt(30) == 0 && event.getType() == EventType.FLOWERS) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (vine.canPlaceBlockAt(world, pos.offset(facing).up(i)))
                    world.setBlockState(pos.offset(facing).up(i), vine.getStateForPlacement(world, pos.offset(facing).up(i), facing, 0, 0, 0, 0, null), 2);
            }
        }
    }
}